...
class AffineTransformWindow(nn.Module):
    ...
    def __init__(
        ...
        if self.use_buffer:
            ...
            index = get_relative_position_index_simple(window_size, anchor_window_down_factor)
    ...
    def forward(self, attn, x_size):
        ...
        if self.use_buffer:
            ...
            table = get_relative_coords_table_all(
                self.window_size, self.pretrained_window_size, self.anchor_window_down_factor
            ).to(device)
            index = get_relative_position_index_simple(self.window_size, self.anchor_window_down_factor).to(device)
...
class AffineTransformStripe(nn.Module):
    ...
    def __init__(
        ...
        if self.use_buffer:
            ...
            index = get_relative_position_index_simple(stripe_size, anchor_window_down_factor, window_to_anchor)
            self.register_buffer("relative_coords_table", table)
            self.register_buffer("relative_position_index", index)

            if self.stripe_shift:
                attn_mask = calculate_mask_all(
                    input_resolution, stripe_size, shift_size, anchor_window_down_factor, window_to_anchor
                )
            else:
                attn_mask = None
            self.register_buffer("attn_mask", attn_mask)
    ...
    def forward(self, attn, x_size):
        ...
        fixed_stripe_size = self.stripe_groups[0] is None and self.stripe_groups[1] is None
        if not self.use_buffer or (self.use_buffer and self.input_resolution != x_size and not fixed_stripe_size):
            # during test and stripe size is not fixed.
            pretrained_stripe_size = self.pretrained_stripe_size  # or stripe_size; Needs further pondering
            table = get_relative_coords_table_all(stripe_size, pretrained_stripe_size, self.anchor_window_down_factor)
            table = table.to(device)
            index = get_relative_position_index_simple(stripe_size, self.anchor_window_down_factor, self.window_to_anchor).to(device)
        else:
            table = self.relative_coords_table
            index = self.relative_position_index
        ...
        if self.use_buffer:
            ...
            if self.input_resolution != x_size and self.stripe_shift > 0:
                mask = calculate_mask_all(x_size, stripe_size, shift_size, self.anchor_window_down_factor, self.window_to_anchor)
                mask = mask.to(device)
        else:
...
class StripeAttention(Attention):
    ...
    def forward(self, qkv, x_size):
        ...
        running_stripe_size, running_shift_size = self.attn_transform._get_stripe_info(x_size)
        # cyclic shift
        if self.stripe_shift:
            qkv = torch.roll(qkv, shifts=(-running_shift_size[0], -running_shift_size[1]), dims=(1, 2))
...
class SeparableConv(nn.Sequential):
    def __init__(self, in_channels, out_channels, kernel_size, stride, bias, args):
        m = [
            nn.Conv2d(in_channels, in_channels, kernel_size, stride, kernel_size // 2, groups=in_channels, bias=bias)
        ]
    ...
    def __init__(self, dim, proj_type, one_stage, anchor_window_down_factor, args):
        ...
        if one_stage:
            if proj_type == "patchmerging":
                ...
                m = AnchorLinear(dim, dim // 2, anchor_window_down_factor, proj_type, True)
            self.body.append(m)
        else:
...
class MixedAttention(nn.Module):
    ...
    def __init__(
        ...
        if self.args.double_window:
            self.stripe_attn = WindowAttention(input_resolution, window_size, num_heads_w, window_shift, attn_drop, pretrained_window_size, args)
        else:
            if self.use_anchor:
                ...
                if self.args.stripe_square:
                    self.stripe_attn = StripeAttention(input_resolution, window_size, [None, None], window_shift, num_heads_s, attn_drop, pretrained_stripe_size, args)
                else:
                    self.stripe_attn = StripeAttention(input_resolution, stripe_size, stripe_groups, stripe_shift, num_heads_s, attn_drop, pretrained_stripe_size, args)
...
class MixAttnTransformerBlock(nn.Module):
    ...
    def forward(self, x, x_size):
        # Mixed attention
        if self.args.local_connection:
            x = x + self.res_scale * self.drop_path(self.norm1(self.attn(x, x_size))) + self.conv(x, x_size)
        else:
